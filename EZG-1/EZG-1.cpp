...
void drawKDTree(node* node, glm::mat4 view, glm::mat4 projection)
{
	if (node->AABB == nullptr) return;

	drawLineBox(node->AABB->mMinPoint, node->AABB->mMaxPoint, lampShader, view, projection, lightVAO);
...
void renderScene(const Shader& shader)
{
	...
	for (int i = 1; i < 10; i++) {
		for (int j = 1; j < 10; j++) {
			...
			if (isFirstRender) masterScene.addSceneObject(cube, model, 1, nullptr);
			cube->Draw(shader);
		}
	}
	...
	model = glm::mat4(1.0f);
	model = glm::translate(model, glm::vec3(6.0, -0.5f, 4.0));
	model = glm::rotate(model, glm::radians(-90.0f), glm::vec3(0.0f, 1.0f, 0.0f));
	model = glm::scale(model, glm::vec3(0.2f));
	shader.setMat4("model", model);
	if (isFirstRender) masterScene.addSceneObject(nanosuit, model, 1, nullptr);
	nanosuit->Draw(shader);

	model = glm::mat4(1.0f);
	model = glm::translate(model, glm::vec3(8.0, -0.5f, 6.0));
	model = glm::scale(model, glm::vec3(0.2f));
	shader.setMat4("model", model);
	if (isFirstRender) masterScene.addSceneObject(nanosuit, model, 1, nullptr);
	nanosuit->Draw(shader);
	...
	model = glm::mat4(1.0f);
	model = glm::translate(model, glm::vec3(10.0, -0.5f, 4.0));
	model = glm::rotate(model, glm::radians(90.0f), glm::vec3(0.0f, 1.0f, 0.0f));
	model = glm::scale(model, glm::vec3(0.2f));
	shader.setMat4("model", model);
	if (isFirstRender) masterScene.addSceneObject(nanosuit, model, 1, nullptr);
...
void processInput(GLFWwindow* window)
{
	...
	if (glfwGetKey(window, GLFW_KEY_RIGHT) == GLFW_PRESS)
		bumpiness = fmin(1.0f, bumpiness + 0.1f);
	if (glfwGetKey(window, GLFW_KEY_LEFT) == GLFW_PRESS)
		bumpiness = fmax(0.0f, bumpiness - 0.1f);
}
